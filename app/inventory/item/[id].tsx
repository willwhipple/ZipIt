...
export default function EditItemScreen() {
  ...
  const fetchData = useCallback(async () => {
    const [itemResult, activitiesResult, itemActivitiesResult] = await Promise.all([
      supabase.from('items').select('*').eq('id', id).single(),
      supabase.from('activities').select('*').order('name'),
      supabase.from('item_activities').select('activity_id').eq('item_id', id),
    ]);
    ...
    if (activitiesResult.error) Alert.alert('Error', activitiesResult.error.message);
    if (itemActivitiesResult.error) Alert.alert('Error', itemActivitiesResult.error.message);
    ...
    setSelectedActivityIds((itemActivitiesResult.data ?? []).map((r) => r.activity_id));
    setLoading(false);
  }, [id]);
  ...
  async function handleSave() {
    ...
    // Replace activity associations: delete all then re-insert
    ...
    if (selectedActivityIds.length > 0) {
      const { error: insertError } = await supabase
        .from('item_activities')
        .insert(selectedActivityIds.map((activity_id) => ({ item_id: id, activity_id })));
      ...
      >
        <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View className="flex-row items-center justify-between px-5 pt-4 pb-2">
            ...
            <Text className="text-lg font-semibold text-gray-900">Edit Item</Text>
